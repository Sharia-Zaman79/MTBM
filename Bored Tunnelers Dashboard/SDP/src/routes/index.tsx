...
import { CheckCircle2, Bell, Trash2, X } from "lucide-react";
      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-left">
            <h1 className="text-2xl font-bold text-white">Bored Tunnelers</h1>
...
function App() {
  ...
  const toggleControl = useCallback((key: keyof Controls) => {
    setControls((prev) => ({ ...prev, [key]: !prev[key] }));
  }, []);

  // Update a specific sensor value
  const updateSensorValue = useCallback(
    (sensorKey: keyof SensorData, newValue: number) => {
      setSensorData((prev) => ({
        ...prev,
        [selectedSensor]: {
          ...prev[selectedSensor],
      ...
      <div className="grid grid-cols-[200px_1fr] lg:grid-cols-[250px_1fr] xl:grid-cols-[300px_1fr] gap-4 lg:gap-6 p-4 lg:p-8">
        ...
        <div className="bg-black rounded-lg p-4 lg:p-6 border border-gray-800">
          ...
          <div className="flex flex-col gap-2 lg:gap-3">
            {(["Pressure", "Temperature", "Current"] as SensorType[]).map(
              (sensor) => (
                <button
                  key={sensor}
...
function SensorBar({
  ...
  unit: string;
  sensorType: string;
  onValueChange: (sensorKey: keyof SensorData, value: number) => void;
}) {
