...
import { useAlerts, type Alert } from "@/lib/alert-store";
import {
  Bell,
  Trash2,
  X,
...
interface IMUData {
  pitch: number; // degrees, -30 to +30 for TBM
  roll: number; // degrees, -15 to +15 for TBM
  heading: number; // degrees, 0-360
  yawRate: number; // degrees per second
  gradient: number; // percentage, -5 to +5
  chainage: number; // meters
}
...
type FaultType = "gyro" | "temp" | null;

function NavigationPage() {
  const chainageRef = useRef(1250.5); // Starting chainage in meters

  const [imuData, setImuData] = useState<IMUData>({
    pitch: 1.2, // Slight downward pitch typical for TBM
    roll: 0.3,
    heading: 127.5, // Southeast heading
    yawRate: 0.02,
    gradient: -0.8, // Slight downward gradient
    chainage: chainageRef.current,
  });

  const [history, setHistory] = useState<HistoryPoint[]>([]);
  const [targetHeading] = useState(128.0); // Design heading
  const [targetGradient] = useState(-1.0); // Design gradient
  const [selectedFault, setSelectedFault] = useState<FaultType>("gyro"); // Default to gyro drift
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setImuData((prev) => {
        // Gradually advance chainage (simulating TBM progress)
  ...
  // Calculate deviations from design alignment
          ...
          <div className="grid grid-cols-2 gap-4 h-full">
            ...
            <div className="bg-gray-900/50 rounded-lg p-4 lg:p-6 border border-gray-800 flex flex-col items-center justify-center">
              ...
              <div className="w-[180px] h-[180px] lg:w-[200px] lg:h-[200px]">
                <AttitudeIndicator
                  roll={imuData.roll * 5} // Scale up for visibility
                  pitch={imuData.pitch * 3} // Scale up for visibility
                  showBox={false}
                />
              </div>
