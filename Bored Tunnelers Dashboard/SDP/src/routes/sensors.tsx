...
type SensorType = "Temperature" | "Pressure" | "Current";
...
function fluctuate(value: number, range: number = 2): number {
...
function SensorsPage() {
  ...
  const [sensorData, setSensorData] = useState<
    Record<SensorType, SensorValues>
  >({
    Temperature: {
      driveMotor: 75,
      slurryPump: 48,
  ...
  const [history, setHistory] = useState<Record<SensorType, HistoryPoint[]>>({
    Temperature: [],
    Pressure: [],
    Current: [],
  });
  ...
  useEffect(() => {
    const initialHistory: Record<SensorType, HistoryPoint[]> = {
      Temperature: [],
      Pressure: [],
      Current: [],
    };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSensorData((prev) => {
        const newData = { ...prev };
        (Object.keys(newData) as SensorType[]).forEach((type) => {
      ...
      setHistory((prev) => {
        ...
        (Object.keys(newHistory) as SensorType[]).forEach((type) => {
  ...
  const updateSensorValue = useCallback(
    (sensorKey: keyof SensorValues, newValue: number) => {
      setSensorData((prev) => ({
        ...prev,
        [selectedSensor]: {
          ...prev[selectedSensor],
      ...
      <div className="flex-1 p-4 lg:p-8 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            {(["Temperature", "Pressure", "Current"] as SensorType[]).map(
              (type) => (
                <Button
                  key={type}
...
function GaugeCard({
  ...
  label: string;
  sensorKey: keyof SensorValues;
  value: number;
  max: number;
  unit: string;
  sensorType: string;
  onValueChange: (sensorKey: keyof SensorValues, value: number) => void;
}) {
